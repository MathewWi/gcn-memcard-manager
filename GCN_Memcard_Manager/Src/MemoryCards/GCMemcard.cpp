...
GCMemcard::GCMemcard(const std::string& filename, bool forceCreation, bool sjis, u16 _sizeMb)
	: m_valid(false)
	, mci_offset(0)
	, m_fileName(filename)
{
	...
	if (!mcdFile.IsOpen())
	{
		if (!forceCreation && !AskYesNoT("\"%s\" does not exist.\n Create a new %d-block Memcard?", filename.c_str(), (_sizeMb*MBIT_TO_BLOCKS)-MC_FST_BLOCKS))
		{
			return;
		}
